...
export default function Register(){
    ...
    const { register, handleSubmit,getValues, formState:{errors}} = useForm<{password:string,confirm_password:string}>();
    ...
    const onSubmit = async (data:any)=>{
        setIsLoading(true)
        userService.resetPassword({token:searchParams.get('token')?.toLocaleLowerCase()||"",
            ...
            <div className=" w-full lg:w-3/4 xl:w-1/2 lg:bg-white flex justify-between">
                ...
                <div className="flex-1 flex flex-col items-center justify-center gap-4 mb-8 py-12 px-10 relative">
                    ...
                    <div className="flex flex-col items-center">
                        <h3 className="font-montserrat font-medium text-[23px] text-gray-900 text-center dark:text-white my-8 tracking-[12px]">Reset Password</h3>
                        <span className="w-full text-center mb-8 font-normal text-[14px] text-gray-700 dark:text-gray-300">Please enter your new password</span>
                     <form onSubmit={handleSubmit(onSubmit)} className="w-full">
                        ...
                        />
                         {
                      isMatch?"":<span className="text-red-600 text-sm">password is not match</span>
                    }
                        <button 
                        type="submit"
                        disabled={!isMatch}
                        className="w-full bg-black text-white text-center py-2 mt-5 font-medium rounded-lg hover:bg-black/80 transition-all duration-300 cursor-pointer">
                            {isLoading?<span className="inline-block w-6 h-6 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></span>:"Reset Password"}
